...
class tp_enrollments {
    ...
    */
     ...
     * @param int $user_id          The user id
     * @param boolean $is_sign_out  Defines if there is a signout option for users (true) or not (false)
     * @param array $settings       The settings array (date_format, order_parent, order_child, order_signups)
     * @return string
     * @since 5.0.0
     * @access private
     */
     ...
     * @access private
     */
    private static function get_results_tab($user_id, $settings) {
        $url = self::get_url();
        $course_id = ( isset($_GET['course_id']) ) ? intval($_GET['course_id'] ) : 0;
        if ( $course_id !== 0 ) {
            return self::get_results_details($course_id, $user_id, $url);
        }
        return self::get_results_overview($user_id, $url, $settings);
    }
     ...
     * @param int $user_id      The user ID
     * @param string $url       The url for the page, where the shortcode is used
     * @param array $settings   The settings array (date_format, order_parent, order_child, order_signups)
     * @since 5.0.0
     * @access private
     */
    ...
    public static function get_interface_for_users($user_id, $user_login, $user_email, $user_exists, $tab, $settings){
        ...
        if ( $tab === 'old' ) {
            $rtn .= self::get_old_tab($user_id, $is_sign_out, $settings); 
        }
        
        // Results / Assessments
        if ( $tab === 'results'  ) {
            $rtn .= self::get_results_tab($user_id, $settings);
        }
...
function tp_enrollments_shortcode($atts) {
    ...
    $settings = array (
        ...
        'order_signups' => esc_sql($order_signups)
    );
